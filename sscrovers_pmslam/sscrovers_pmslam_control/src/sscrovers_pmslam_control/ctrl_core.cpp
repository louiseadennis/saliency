...
void CtrlCore::publishCtrlVector()
{
  ...
  float d2 = curr_pose_msg_.pose.position.x*curr_pose_msg_.pose.position.x + curr_pose_msg_.pose.position.y*curr_pose_msg_.pose.position.y;
  float d = sqrt(d2);
ROS_INFO("%f,%f",curr_pose_msg_.pose.position.x,curr_pose_msg_.pose.position.y );
  float theta = 0;
  if(curr_pose_msg_.pose.position.x *curr_pose_msg_.pose.position.x<0.051){
	theta = atan(curr_pose_msg_.pose.position.y/curr_pose_msg_.pose.position.x);
  }
  ctrl_vec_msg_.d = d;
